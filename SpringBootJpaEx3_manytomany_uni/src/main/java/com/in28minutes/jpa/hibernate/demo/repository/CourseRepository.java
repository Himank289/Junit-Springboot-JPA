...
public Faculty findFacultyOfCourse(int courseid) {
	Course c=this.findById(courseid);
	Faculty f=c.getFaculty();
	if(f!=null) {
		logger.info("Faculty teaching ourse"+c.getCourseName()+"is"+f.getFacultyName());
	}
	else {
		logger.info("Faculty teaching ourse"+c.getCourseName()+"is"+f.getFacultyName()+"is null");
	}
