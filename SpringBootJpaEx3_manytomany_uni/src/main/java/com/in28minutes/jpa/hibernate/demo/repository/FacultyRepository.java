...
public List<Course> getAllCourses(int facultyId){
	Faculty f=this.findById(facultyId);
	List<Course> lst1=f.getCourses();
//	logger.info();
	lst1.forEach(System.out::println);
	return lst1;
}

public List<Course> getAllCoursesOfFaculty(int id){
		Faculty f=this.findById(id);
		List<Course> lst=f.getCourses();
		logger.info("faculty"+f.getFacultyName()+"is teaching following courses"+lst);
		return lst;
